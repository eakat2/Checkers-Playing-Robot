...
from .constants import DARK, LIGHT, BLACK, ROWS, COLS, SQUARE_SIZE, FORCE_TAKE, KING_TAKE, KING_MOVE
        ...
        if self.red_left <= 0:
            ...
            for row in range(ROWS):
                for col in range(COLS):
                    ...
                    if piece != 0 and piece.colour == DARK:
                        if self.get_moves_list(piece):
                            red_valid = True
                    elif piece != 0 and piece.colour == LIGHT:
                        if self.get_moves_list(piece):
                            white_valid = True
