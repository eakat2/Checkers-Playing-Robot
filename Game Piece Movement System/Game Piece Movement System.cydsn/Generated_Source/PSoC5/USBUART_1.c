...
#include "USBUART_1_PVT.h"
    ...
    uint32 USBUART_1_pinNumber = USBUART_1_PIN_NUMBER;
    uint32 USBUART_1_pinPortNumber = USBUART_1_PIN_PORT_NUMBER;
    #if(CY_PSOC3)
        uint32 pdata USBUART_1_pinDrAdress = USBUART_1_PIN_DR;
    #else
        uint32 USBUART_1_pinDrAdress = USBUART_1_PIN_DR;
    #endif /* (CY_PSOC3) */
#else
    uint32 USBUART_1_pinNumber;
    uint32 USBUART_1_pinPortNumber;
    #if(CY_PSOC3)
        uint32 pdata USBUART_1_pinDrAdress;
    #else
        uint32 USBUART_1_pinDrAdress;
    #endif /* (CY_PSOC3) */
...
#else
    /*******************************************************************************
    * Function Name: USBUART_1_StartEx
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    void USBUART_1_StartEx(uint8 port, uint8 pin) 
    {
        ...
        if ((pin <= USBUART_1_MAX_PIN_NUMBER) && (port <= USBUART_1_MAX_PORT_NUMBER))
        {
            #if (!CY_PSOC4)
                portConfigAddr = USBUART_1_PORT_CNF_BASE;
                portConfigAddr += ((uint32)port * (USBUART_1_MAX_PIN_NUMBER + 1u)) + pin;
                CyPins_SetPinDriveMode(portConfigAddr, CY_PINS_DM_STRONG);
                CyPins_SetPin(portConfigAddr);
                USBUART_1_pinDrAdress = portConfigAddr;
            #else
                portConfigAddr = USBUART_1_PORT_CNF_BASE + (USBUART_1_PORT_CNF_SIZE * port) +
                                                                                USBUART_1_PORT_CNF_MODE_OFFSET;
                CY_SYS_PINS_SET_DRIVE_MODE(portConfigAddr, pin, CY_SYS_PINS_DM_STRONG);
                portDataRegAddr = USBUART_1_PORT_CNF_BASE + (USBUART_1_PORT_CNF_SIZE * port) +
                                                                                USBUART_1_PORT_CNF_DR_OFFSET;
                CY_SYS_PINS_SET_PIN(portDataRegAddr, pin);
                USBUART_1_pinDrAdress = portDataRegAddr;
            #endif /* (!CY_PSOC4) */
            USBUART_1_pinNumber = pin;
            USBUART_1_pinPortNumber = port;
        }
    }
#endif /* (USBUART_1_PIN_STATIC_MODE == 1u) */
...
void USBUART_1_PutString(const char8 string[]) 
{
    ...
    {
        USBUART_1_PutChar((uint8)current);
...
/*******************************************************************************
* Function Name: USBUART_1_PutArray
********************************************************************************
*
* Summary:
...
void USBUART_1_PutArray(const uint8 array[], uint32 byteCount) 
{
    ...
    {
        USBUART_1_PutChar(array[arrayIndex]);
    }
}


/*******************************************************************************
* Function Name: USBUART_1_PutHexByte
********************************************************************************
*
* Summary:
...
{
    static char8 const CYCODE USBUART_1_hex[] = "0123456789ABCDEF";

    USBUART_1_PutChar((uint8) USBUART_1_hex[txHexByte >> USBUART_1_BYTE_UPPER_NIBBLE_SHIFT]);
    USBUART_1_PutChar((uint8) USBUART_1_hex[txHexByte & USBUART_1_BYTE_LOWER_NIBBLE_MASK]);
}


/*******************************************************************************
* Function Name: USBUART_1_PutHexInt
********************************************************************************
*
* Summary:
...
{
    USBUART_1_PutHexByte((uint8)(txHexInt >> USBUART_1_U16_UPPER_BYTE_SHIFT));
    USBUART_1_PutHexByte((uint8)(txHexInt & USBUART_1_U16_LOWER_BYTE_MASK));
}
...
{
    USBUART_1_PutChar(0x0Du);
    USBUART_1_PutChar(0x0Au);
}
